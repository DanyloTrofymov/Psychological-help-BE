...
export class ChatroomService {
	...
	async createChatroomAI(
		...
		return await this.prismaService.chatroom.create({
			data: {
				title,
				ChatroomParticipants: { create: { userId, aiThreadId } },
				withAI
			}
		});
	}
	...
	async createChatroomUsers(title: string, userId: number): Promise<Chatroom> {
		return await this.prismaService.chatroom.create({
			data: {
				title,
				ChatroomParticipants: { create: { userId } }
			}
		});
	}
	...
	async getChatroomWithoutTherapist(page: number, pageSize: number) {
		...
		const take = pageSize;
		const totalElements = await this.prismaService.chatroom.count({
			where: { active: true }
		});
		...
		const filteredChatrooms = chatrooms.filter(
			chatroom => chatroom.ChatroomParticipants.length === 1 && !chatroom.withAI
		);
	...
	async joinChatroom(chatroomId: number, userId: number, role?: ROLE) {
		...
		if (
			role == ROLE.THERAPIST &&
			(room.withAI || room.ChatroomParticipants.length > 1)
		) {
	...
	async pingAdmin(chatroomId: number, userId: number) {
		await this.addUserMessage(
			chatroomId,
			userId,
			'Запит на підключення адміністратора відправлено'
		);
	...
	async getAIChatrooms(userId: number, page: number, pageSize: number) {
		...
		const totalElements = await this.prismaService.chatroom.count({
			where: {
				ChatroomParticipants: { some: { userId } },
				withAI: true,
				active: true
			}
		});
	...
	async getMessagesByChatroomId(
		...
		const skip = page * pageSize;
		const take = pageSize;
		const totalElements = await this.prismaService.chatroomMessages.count({
			where: { chatroomId }
		});
		const totalPages = Math.ceil(totalElements / pageSize);

		const messages = await this.prismaService.chatroomMessages.findMany({
			where: { chatroomId },
	...
	async renameChatroom(
		...
		if (
			!chatroom.ChatroomParticipants.some(p => p.userId == user.id) &&
	...
	async sendTakeToAi(takeId: number, userId: number) {
		...
		const AImessage = await this.openai.beta.threads.messages.create(
			thread.id,
			{ role: 'user', content: message }
		);

		const AIchatroom = await this.createChatroomAI(
			'Асистент',
			userId,
			thread.id,
			true
	...
	async getResultString(takeId: number) {
		const take = await this.prismaService.take.findUnique({
			where: { id: takeId },
			include: {
				answers: true,
		...
		const answers = take.answers.map(answer => {
			const question = take.quiz.questions.find(
				q => q.id === answer.questionId
			);
			return {
				question,
				answer,
				score: question.answers.find(a => a.id === answer.answerId).score
			};
		});
		const answersString = answers
			.map(a => `${a.question.title} - ${a.answer.answerId} - ${a.score} балів`)
			.join('\n');
