...
export class ChatroomController {
	...
	async getAiChats(
		...
		return await this.chatroomService.getAIChatrooms(
			req.user.id,
			page,
			pageSize
		);
	...
	async getUsersChats(
		...
		return await this.chatroomService.getUsersChatrooms(
			req.user.id,
			page,
			pageSize
		);
	...
	@Get('withoutTherapis')
	async getWithoutTherapis(
		@Query('page', ParseIntPipe) page: number = 0,
		@Query('pageSize', ParseIntPipe) pageSize: number = 10
	) {
		return await this.chatroomService.getChatroomWithoutTherapist(
			page,
			pageSize
		);
	}
	...
	async joinRoom(@Request() req, @Body() body: { chatroomId: number }) {
		return await this.chatroomService.joinChatroom(
			body.chatroomId,
			req.user.id,
			req.user.role
		);
	...
	async getMessages(
		...
		return await this.chatroomService.getMessagesByChatroomId(
			chatroomId,
			page,
			pageSize
		);
